...
class Search extends Module
{
  ...
  static function getResult() {
    ...
    if (!empty($find_words) && strlen($find_words) > 2) {
      ...
      $find = $DB->getAll('SELECT SQL_CALC_FOUND_ROWS m.id, m.name, p.content
                           FROM '.getTablePrefix().'menu AS m
                           INNER JOIN '.getTablePrefix().'page AS p ON p.menu_id = m.id
                           WHERE p.content LIKE \'%' . $find_words .'%\' OR
                           m.name like \'%' . $find_words .'%\' LIMIT ' . $limit);
      ...
      if (!empty($find)) {
        ...
        foreach ($find as $item) {
          ...
          $content .= '<a href="/?cat_id=' . $item['id'] .'"><b>'. $item['name'] . '</b> ' . $resultText . '</a><br/><br/>';
        }
