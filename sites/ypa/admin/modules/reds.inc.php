...
elseif (!empty($_FILES['img']['tmp_name']))
{
    ...
    if (!empty($name))
    {
        ...
        if (is_uploaded_file($tmp_name))
        {
            ...
            if (glob($root_path.'i/service/'.$name)) $name = time().$r;
            $thumb_path =  $root_path.'i/service/'.$name;

            if (!resizeimg($tmp_name, $thumb_path, $info[0], $info[1], $info[2], $imagesize['service']))
            {
