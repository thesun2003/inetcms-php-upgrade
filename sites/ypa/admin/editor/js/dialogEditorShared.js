...
function wproOpenFileBrowser(type, returnFunction, getFunction, baseurl, sid, iframe, phpsid, appendToQueryStrings) {
if (!type) type = 'link';
WPRO_FB_RETURN_FUNCTION[sid] = returnFunction
WPRO_FB_GET_FUNCTION[sid] = getFunction
var url = baseurl + 'dialog.php?dialog=wproCore_fileBrowser&action=link&chooser=true&dirs='+type + '&' + sid + (phpsid ? '&' + phpsid : '') + (appendToQueryStrings ? '&' + appendToQueryStrings : '');
if (iframe) {
var frame = 'wpfileBrowser_dialogFrame';
}
wp_openDialog(url, 'modal', 760, 480-55, '', frame);
}
function wpro_sessTimeout (internalId, sid, phpsid, url, appendToQueryStrings, sessRefresh) {
var tag
if (tag = document.getElementById(internalId+'_sessTag')) {
tag.parentNode.removeChild(tag);
}
setTimeout('try{wpro_createSessTag("touch", "'+wproAddSlashes(internalId)+'", "'+wproAddSlashes(sid)+'", "'+wproAddSlashes(phpsid)+'", "'+wproAddSlashes(url)+'", "'+wproAddSlashes(appendToQueryStrings)+'")}catch(e){}', sessRefresh*1000);
}
function wpro_createSessTag (action, internalId, sid, phpsid, url, appendToQueryStrings) {
...
var srcStr = url + 'core/touch.php?' + sid + (phpsid ? '&' + phpsid : '');
if (action == 'destroy') {
...
win = wproWinOpen(url, name[1], "modal=yes,width="+width+"px,height="+height+"px,"+features+","+params);
}
