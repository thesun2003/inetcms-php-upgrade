...
return '<script type="text/javascript" src="'+editor._domain+WPro.URL+'js/wproObject.js"></script>\n<script type="text/javascript">wproObject.write('+str+');</script>';
}
...
if (title) {
data = editor.plugins['wproCore_fileBrowser'].unserializeMedia(WPro.htmlSpecialCharsDecode(title[0]), 'source');
var str = '';
if (data['object']) {
for (var x in copy) {
if (copy[x]) {
...
if (alt) str+=' alt="'+alt+'"';
str += ' _wpro_media_data="'+WPro.htmlSpecialChars(mediaData)+'" />';
return str;
}
