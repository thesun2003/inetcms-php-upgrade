...
class wproDialog extends wproCore {
	...
	function registerAjaxFunction($function) {
		
		if (!$this->ajax) {
			$this->loadAjax();
			$this->ajax->sRequestURI = preg_replace("/action=[a-z]+/smi", "action=ajax", $this->ajax->_detectURI());
		...
		$this->ajax->registerFunction($function);
	...
	function loadPlugin($name, $default=false) {
		...
		if (!isset($this->plugins[$name])) {
			...
			// load componant file
			if (!class_exists("wproDialogPlugin_{$name}")) {
	...
	function display() {
		...
		if ($this->ajax) {
			if ($this->ajax->canProcessRequests()) {
				register_shutdown_function('wproDialogShutdown');
