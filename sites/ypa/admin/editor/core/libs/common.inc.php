...
function xajaxCatchSessionTimeout () {
...
function xajaxCatchUnauthorized () {
	require_once(WPRO_PATH_XAJAX."xajaxResponse.inc.php");
	$response = new xajaxResponse();
...
if (!$EDITOR = $WPRO_SESS->load()) {
	...
	if (isset($_REQUEST[$WPRO_SESS->sessionName]) && !WPRO_ANONYMOUS_ACCESS) {
		if (isset($_POST['xajax'])) {
			...
			$xajax = new xajax();
			$xajax->registerCatchAllFunction("xajaxCatchSessionTimeout");
		...
		if (isset($_POST['xajax'])) {
			require_once(WPRO_PATH_XAJAX."xajax.inc.php");
			$xajax = new xajax();
			$xajax->registerCatchAllFunction("xajaxCatchUnauthorized");
			$xajax->processRequests();
		} else {
