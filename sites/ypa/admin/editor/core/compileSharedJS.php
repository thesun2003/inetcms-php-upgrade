<?php
define('IN_WPRO', true);
define('WPRO_DIR', dirname(dirname(__FILE__)) . '/');
...
if (typeof(wproAjaxRecordLoad) != 'undefined') {
	wproAjaxRecordLoad('<?php echo strtolower(addslashes($_SERVER['PHP_SELF'] . (empty($_SERVER['QUERY_STRING']) ? '' : '?'.$_SERVER['QUERY_STRING']) )) ?>');
}