...
function xajaxContainsValue(arr,valueToCheck){for(var i=0;i<arr.length;i++){if (arr[i]==valueToCheck)return true;}return false;}
...
this._nodeToObject=function(node){if (!node) return;if(node.nodeName=='#cdata-section'){var data="";for(var j=0;j<node.parentNode.childNodes.length;j++){data+=node.parentNode.childNodes[j].data;}
return data;}
...
else{if(xajaxContainsValue(xajax.responseErrorsForAlert,r.status)){var errorString="Error: the server returned the following HTTP status: "+r.status;errorString+="\nReceived:\n"+r.responseText;alert(errorString);}
