...
/*
 * WysiwygPro 3.0.0.20071005 (c) Copyright Chris Bolt and ViziMetrics Inc. All Rights Reserved.
...
while (html.match(/<[^>]+="[^"]*[<>][^"]*"/g)) {
html = html.replace(/<[^>]+="[^"]*[<>][^"]*"/g, function(x){var a = x.replace(/<[^>]+="([^"]*[<>][^"]*)"/g, '$1').replace(/>/g, '&gt;').replace(/</g, '&lt;');  return x.replace(/(<[^>]+=)"[^"]*[<>][^"]*"/g, '$1"'+a+'"'); });
}
}
