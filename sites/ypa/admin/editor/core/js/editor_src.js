...
wproObj.prototype.callCommand = function (doc, command, gui, value) {
	...
	return eval('doc.e'+'xec'+'Com'+'ma'+'nd(command, gui, value)');
}
...
wproObj.prototype.escapeServerTags = function (html) {
	...
	while (html.match(/(<[^>]*)(<(\%|\?[^x])[\s\S]*?(\%|\?)>)/gi) ) {
		html = html.replace(/(<[^>]*)(<(\%|\?[^x])[\s\S]*?(\%|\?)>)/gi, function(x){
			...
			var a = x.replace(/^<[^>]*</i, '<');
			return b + '[WPSCODE'+escape(a)+'WPSCODE]';
		});
	}
	// escape tags within text.
	while (html.match(/(>[^<]*)(<(\%|\?[^x])[\s\S]*?(\%|\?)>)/gi) ) {
		html = html.replace(/(>[^<]*)(<(\%|\?[^x])[\s\S]*?(\%|\?)>)/gi, "$1<!--[WPCOMMENT$2WPCOMMENT]-->");
	}
	return html;
}
...
//alert(div.innerHTML);
	...
	//if (!WPro.isSafari) {
		...
		//obj.previewDocument= obj.previewWindow.document;
	//}
	...
	if (obj.textarea.form) WPro.events.addEvent(obj.textarea.form, 'submit', WPro.eval('WPro.'+obj._internalId+'.prepareSubmission'));
...
wproEditor.prototype.redraw = function () {
	if (!this._inPreview && !this._movingToPreview) {
		//this.editDocument.body.style.display='none';
		//this.editDocument.body.style.display='';
...
wproEditor.prototype.urlFormatting = function (url) {
	...
	var locationRegex = new RegExp('^'+WPro.quoteMeta(this._location.replace(/&/gi,'(&|&amp;)'))+'#', 'gi');
	...
	if (this.fullURLs) {
		...
		var domainRegex = new RegExp('^'+WPro.quoteMeta(loc) +'[^\/]*(|/)', 'gi');
...
wproEditor.prototype.sourceFormatting = function (html) {
	...
	if (WPro.isIE) {
		while (html.match(/<[^>] [a-z_]+='[^']*'/g)) {
			html = html.replace(/<[^>] [a-z_]+='[^']*'/g, function(x){var a = x.replace(/<[^>] [a-z_]+='([^']*)'/g, '$1').replace(/"/g, '&quot;').replace(/>/g, '&gt;').replace(/</g, '&lt;'); return x.replace(/(<[^>] [a-z_]+=)'[^']*'/g, '$1"'+a+'"'); });
		}
		while (html.match(/<[^>]+="[^"]*[<>][^"]*"/g)) {
			html = html.replace(/<[^>]+="[^"]*[<>][^"]*"/g, function(x){var a = x.replace(/<[^>]+="([^"]*[<>][^"]*)"/g, '$1').replace(/>/g, '&gt;').replace(/</g, '&lt;'); /*alert(x.replace(/(<[^>]+=)"[^"]*[<>][^"]*"/g, '$1"'+a+'"'));*/ return x.replace(/(<[^>]+=)"[^"]*[<>][^"]*"/g, '$1"'+a+'"'); });
		}
	}
	...
	if (results) {
		...
		for (var i=0; i < rl; i++) {
			...
			// url formatting, but leave base tags alone
			if (tagName!='base') {
	...
	var num2 = this.stylesheets.length;
	for (var j=0; j < num2; j++) {
		if (this.stylesheets[j] != '') {
			html = eval ('html.replace(/<link[^>]* href="' + WPro.quoteMeta(this.urlFormatting(this.stylesheets[j])) + '"[^>]*>/gi, "")');
		}
	}
	if (this.defaultCSS != '') {
	...
	if (this.escapeCharacters) {
		html = WPro.escapeCharacters(html, this.escapeCharactersRange);
	} else {
...
wproEditor.prototype._buildPreviewHTML = function (html,docStyles) {
	...
	for (var i=0; i < num; i++) {
		if (this.stylesheets[i] != '') {
			head += '<link href="' + this.stylesheets[i] + '" type="text/css" rel="stylesheet">';
		}
	}
	/* add default styles */
	if (this.defaultCSS != '') {
	...
	if (this.snippet) {
		var str = this.doctype + '<ht'+'ml'+htmla+'><he'+'ad><title>Preview</title><meta http-equiv="Content-Type" content="text/html; charset=' + this.charset + '">' + head + '</he'+'ad><bo'+'dy>' + html + '</bo'+'dy></ht'+'ml>';
	} else {
		var str = html.replace(/<head([^>]*)>/gi, "<he"+"ad$1>"+head).replace(/<html([^>]*)>/gi, "<ht"+"ml" + htmla + "$1>");
		//str = head + str;
	}
...
wproEditor.prototype.syntaxHighlight = function (html) {
	...
	var formTag = /(&lt;(form|input|button|select|option|\/form|\/input|\/button|\/select|\/option)([\s\S]*?)&gt;)/gi
...
wproEditor.prototype.getDialogPluginURL = function (url) {
	var url = WPro.URL + 'dialog.php?dialog=' + url + '&' + this.sid + (WPro.phpsid ? '&' + WPro.phpsid : '') + (this.appendToQueryStrings ? '&' + this.appendToQueryStrings : '');
	return url;
}
...
wproEditor.prototype.showTagEditor = function (usetagpath, tagIndex) {
	...
	if (tagName == 'IMG' && node.className.match(/wproFilePlugin/i)) {
		if (!/'object'\:\{/.test(String(node.getAttribute("_wpro_media_data")))) {
			tagName = 'EMBED';
		} else {
			tagName = 'OBJECT';	
		}
...
wproEditor.prototype.showGuidelines = function (change) {
	...
	if (this._inDesign||this._movingToDesign) {
...
wproEditor.prototype.resizeTo = function (width, height) {
	...
	var toolbarHeight = this.designToolbar.offsetHeight ? this.designToolbar.offsetHeight : (this.toolbarHeight * th.length+2);
...
wproEditor.prototype.callFormatting = function (sFormatString, sValue) {
	...
	if (WPro.isGecko) {
		...
		var t = p.tagName;
		if (t == 'BODY' || (t=='DIV' && this.lineReturns!='div')) {
			//alert(((this.linReturns=='div') ? 'div' : 'p'));
...
wproEditor.prototype.applyStyle = function (elm, nodes, swapNode, preserveAttrs) {
	...
	//if (tagName == 'SPAN' && !nodes) { // inline style
		...
		//alert(range.childNodes);
		
		// find start container
		var startRange = range.cloneRange()
		startRange.collapse(true);
		
		if (beyondBlock.test(tagName) || WPro.inline_tags.test(tagName)) {	
...
wproEditor.prototype._sessTimeout = function () {
	wpro_sessTimeout (this._internalId, this.sid, WPro.phpsid, WPro.URL, this.appendToQueryStrings, this.sessRefresh);
}
wproEditor.prototype._createSessTag = function (action) {
	wpro_createSessTag (action, this._internalId, this.sid, WPro.phpsid, WPro.URL, this.appendToQueryStrings)
}
/* floating Menu */
wproEditor.prototype.showContextMenu = function (evt) {
	...
	if (this._inSource) {var allowed =/^(cut|copy|paste)$/i} else {var allowed = null}
				...
				if (tagName == 'IMG') {
					if (this.imageStyles) {
						...
						for (var i in this.imageStyles) {
							...
							if (!i.match('src=')) {
								i2+=' src="'+this.themeURL+'buttons/image.gif"';
							}
							if (filePlugin) {
								if (i.match('class=')) {
			...
			if (items[i][0] == 'tageditor' || items[i][0] == 'removetag' || items[i][0] == 'deletetag') {
				if (range.type=='control') {
					...
					if (tagName == 'IMG' && range.nodes[0].className.match(/wproFilePlugin/i)) {
						if (!/'object'\:\{/.test(String(range.nodes[0].getAttribute("_wpro_media_data")))) {
							tagName = 'EMBED';
						} else {
							tagName = 'OBJECT';	
						}
...
function wproSFClearEmpty (EDITOR, html){
	...
	if (!WPro.hasContent(EDITOR.editDocument.body)) {
		return '';
	}else{
		return html;
	}
