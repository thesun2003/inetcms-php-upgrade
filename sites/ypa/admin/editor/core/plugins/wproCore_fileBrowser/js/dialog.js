...
form.elements['mediamtop'].value = String(img.style.marginTop).toString().replace(/[^\-0-9]/gi, '');
form.elements['mediambottom'].value = String(img.style.marginBottom).toString().replace(/[^\-0-9]/gi, '');
form.elements['mediamleft'].value = String(img.style.marginLeft).toString().replace(/[^\-0-9]/gi, '');
form.elements['mediamright'].value = String(img.style.marginRight).toString().replace(/[^\-0-9]/gi, '');
}
...
break;
case 'thumbnailsView' :
b.style.display = (type=='image'&&thumbnails) ? '' : 'none';
if (type!='image'||!thumbnails) {
...
var url = dialog.URL + 'dialog.php?dialog=wproCore_fileBrowser&action=preview&' + dialog.sid + (dialog.phpsid ? '&' + dialog.phpsid : '') + (dialog.appendToQueryStrings ? '&' + dialog.appendToQueryStrings : '');
...
var d = [];
if (folders) {
if (folders.length==1&&folders[0]==0) {
...
var url = dialog.URL + 'dialog.php?dialog=wproCore_fileBrowser&action=nopreview&' + dialog.sid + (dialog.phpsid ? '&' + dialog.phpsid : '') + (dialog.appendToQueryStrings ? '&' + dialog.appendToQueryStrings : '');
if (document.getElementById('detailsHolder').style.display!='none')
setTimeout("dialog.changeFrameLocation(document.getElementById('filePanePreview'), '"+dialog.addSlashes(url)+"')", 1);
}
var t = document.createTextNode(' '+details.name);
