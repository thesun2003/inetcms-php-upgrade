...
class wproDialogPlugin_wproCore_fileBrowser {
	...
	function addDialogJS () {
		...
		if (WPRO_COMPILE_JS_INCLUDES) {
			$DIALOG->headContent->add('<script type="text/javascript" src="core/plugins/wproCore_fileBrowser/compileJS.php?v='.$EDITOR->version.'"></script>');
		} else {
	...
	function buildThumbnails ($directory, $url, &$files, $urlBase) {
		return require(dirname(__FILE__).'/processors/buildThumbnails.inc.php');
	}
	...
	function delete($folderId, $folderPath, $files, $page=1, $sortBy='name', $sortDir='asc', $view='thumbnails', $nonce, $history=false) {
		return require(dirname(__FILE__).'/processors/delete.inc.php');		
	}
	
	function rename ($folderId, $folderPath, $files, $page=1, $sortBy='name', $sortDir='asc', $view='thumbnails', $nonce, $history=false) {
		return require(dirname(__FILE__).'/processors/rename.inc.php');
	}
	
	function newFolder ($folderId, $folderPath, $name, $page=1, $sortBy='name', $sortDir='asc', $view='thumbnails', $nonce, $history=false) {
		return require(dirname(__FILE__).'/processors/newFolder.inc.php');
	}
	
	function uploadFinished($folderId, $folderPath, $uploadID, $overwrite=array(), $page=1, $sortBy='name', $sortDir='asc', $view='thumbnails', $history=false) {
		return require(dirname(__FILE__).'/processors/uploadFinished.inc.php');
	}	
	
	function moveCopyFinished($moveCopyID, $overwrite=array(), $page=1, $sortBy='name', $sortDir='asc', $view='thumbnails', $history=false) {
		return require(dirname(__FILE__).'/processors/moveCopyFinished.inc.php');
	}	
	
	function editImage($folderId, $folderPath, $editorID, $task, $options=null) {
		return require(dirname(__FILE__).'/processors/imageeditor.ajax.inc.php');	
	}
	...
	function loadEmbedPlugin($name) {
		...
		if (!isset($this->embedPlugins[$name])) {
			if (!class_exists('wproFilePlugin_'.$name)) {
				$dir = WPRO_DIR.'plugins/mediaPlugins/';
				if (!$fs->includeFileOnce($name, $dir, '/plugin.php')) {
					return false;
				} else if (!class_exists('wproFilePlugin_'.$name)) {
					return false;
				}
			}
	...
	function displayEmbedPluginsJS() {
		...
		if (WPRO_COMPILE_JS_INCLUDES) {
			...
			$DIALOG->headContent->add('<script type="text/javascript" src="core/plugins/wproCore_fileBrowser/compileFilePluginsJS.php?plugins='.base64_encode($s).'&amp;v='.$EDITOR->version.'"></script>');
		} else {
			foreach ($this->embedPlugins as $name => $plugin) {
	...
	function getMediaDimensions ($file) {
		require_once(WPRO_PATH_GETID3.'getid3.php');
		$return = false;
		if (file_exists($file)) {
			$getID3 = new getID3();
	...
	function displayFileDetails($folderId, $folderPath, $file, $fromPlugin=true) {
		global $EDITOR, $DIALOG;
	...
	function displayFolderDetails($folderId, $folderPath, $folder) {
		global $EDITOR, $DIALOG;
	...
	function runAction ($action, $params) {
		...
		switch (strtolower($action)) {
			...
			case 'image' : case 'media' : case 'document' : case 'link' : case 'imageeditor' : case 'ajax':
				...
				$DIALOG->registerAjaxFunction(array('displayFolderList', &$this, 'displayFolderList'));
