...
if (!defined('IN_WPRO')) exit;
				...
				$linksBrowserURL = str_replace(array('?&','&&'), array('?','&'),  (
						...
						'dialog.php?dialog=wproCore_fileBrowser&action=linksBrowser' : 
						
						$EDITOR->linksBrowserURL.(strstr($EDITOR->linksBrowserURL, '?') ? '&' : '?')
					
					
					).($EDITOR->appendToQueryStrings ? '&'.$EDITOR->appendToQueryStrings : '').'&'.$wpsname.'='.$wpsid.strip_tags(defined('SID') ? '&'.SID : ''));
