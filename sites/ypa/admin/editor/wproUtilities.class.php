...
class wproUtilities {	
	...
	function emailEncode($code) {
		$code = preg_replace ("/<a(.*?)href=\"(mailto:.*?)\"(.*?)>(.*?)([a-zA-Z0-9_\.-]+@[a-zA-Z0-9\.-]+\.[a-zA-Z]{2,4})(.*?)<\/a>/e", "'<a' . stripslashes('$1') . 'href=\"' . wproUtilities::_emailEncode2('$2') . '\"' . stripslashes('$3') . '>' . stripslashes('$4') . wproUtilities::_emailEncode2('$5') .  stripslashes('$6') . '</a>'", $code);
		return preg_replace ("/a(.*?)href=\"(mailto:.*?)\"(.*?)>/e", "'a' . stripslashes('$1') . 'href=\"' . wproUtilities::_emailEncode2('$2') . '\"' . stripslashes('$3') . '>'", $code);
	}
