...
class Search extends Module {
  ...
  static function get_google_results() {
    ...
    $content .= SimplePage::process_template_file(
      ...
      array(
        'search_query_utf8' => iconv("windows-1251", "utf-8", $search_query),
  ...
  static function getResult() {
    ...
    if (!empty($find_words) && strlen($find_words) > 2) {
      if (!empty($_GET['str']) && is_numeric($_GET['str'])) {
        $str = $_GET['str'];
      } else {
        $str = 0;
      }
      $limit = 'LIMIT ' . $str . ', '. self::page_count;
      ...
      $find = $DB->getAll('SELECT SQL_CALC_FOUND_ROWS m.*, p.content 
                           ...
                           (p.content != \'\' AND  m.name like \'%' . $find_words .'%\')' . $limit);
      ...
      if (!empty($find)) {
        ...
        foreach ($find as $item) {
          ...
          if (!$resultText) {
            ...
            $content .= '<a href="/?cat_id=' . $item['id'] .'">' . $resultText . '</a><br/><br/>';
          }
        }
